...
class PlayAgainstAI(GameView):
    def __init__(self, best_network):
        ...
        self.ai_bird = Bird(x=INITIAL_BIRD_X, y=INITIAL_BIRD_Y, bird_type="ai")

        self.player_bird = Bird(x=INITIAL_BIRD_X, y=INITIAL_BIRD_Y, bird_type="player")
    ...
    def _make_ai_choose_jump(self):
        ...
        if (
            len(self.pipes) > 1
            and self.ai_bird.x > self.pipes[0].x + self.pipes[0].WIDTH
        ):
            pipe_index = 1

        # Send bird location, top pipe location and bottom pipe location and determine from network whether to jump or not
        output_network = self.best_network.activate(
            (
                self.ai_bird.y,
                abs(self.ai_bird.y - self.pipes[pipe_index].y_top),
                abs(self.ai_bird.y - self.pipes[pipe_index].y_bottom),
            )
        )
